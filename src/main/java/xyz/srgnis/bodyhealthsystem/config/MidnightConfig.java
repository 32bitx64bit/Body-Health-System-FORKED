...
public abstract class MidnightConfig {
    ...
    public static class MidnightConfigScreen extends Screen {
        ...
        public void render(DrawContext drawContext, int mouseX, int mouseY, float delta) {
            ...
            drawContext.drawCenteredTextWithShadow(textRenderer, title, width / 2, 15, 0xFFFFFF);
            ...
            for (EntryInfo info : entries) {
                if (info.id.equals(modid)) {
                    if (list.getHoveredButton(mouseX,mouseY).isPresent()) {
                        ...
                        if (info.error != null && text.equals(name)) drawContext.drawTooltip(textRenderer, info.error.getValue(), mouseX, mouseY);
                        else if (I18n.hasTranslation(key) && text.equals(name)) {
                            List<Text> list = new ArrayList<>();
                            for (String str : I18n.translate(key).split("\n"))
                                list.add(Text.literal(str));
                            drawContext.drawTooltip(textRenderer, list, mouseX, mouseY);
                        }
                    }
                }
            }
    ...
    public static class ButtonEntry extends ElementListWidget.Entry<ButtonEntry> {
        ...
        public void render(DrawContext drawContext, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            buttons.forEach(b -> { b.setY(y); b.render(drawContext, mouseX, mouseY, tickDelta); });
            if (text != null && (!text.getString().contains("spacer") || !buttons.isEmpty())) {
                if (info.centered) drawContext.drawTextWithShadow(textRenderer, text, MinecraftClient.getInstance().getWindow().getScaledWidth() / 2 - (textRenderer.getWidth(text) / 2), y + 5, 0xFFFFFF);
                else drawContext.drawTextWithShadow(textRenderer, text, 12, y + 5, 0xFFFFFF);
            }
        }
