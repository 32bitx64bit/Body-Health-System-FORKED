...
public class HealScreen extends HandledScreen<HealScreenHandler> {
    ...
    protected void drawBackground(DrawContext drawContext, float delta, int mouseX, int mouseY) {
        ...
        drawContext.drawTexture(TEXTURE, startX,startY,0,0,this.backgroundWidth,this.backgroundHeight);
    }

    protected void drawButtons(DrawContext drawContext, float delta, int mouseX, int mouseY) {
        int startX = (this.width - this.backgroundWidth) / 2;
    ...
    public void render(DrawContext drawContext, int mouseX, int mouseY, float delta){
        ...
        drawButtons(drawContext, delta, mouseX, mouseY);
    }

    @Override
    protected void drawForeground(DrawContext drawContext, int mouseX, int mouseY) {
        drawContext.drawText(this.textRenderer, this.handler.getEntity().getName(), this.titleX, this.titleY, 4210752, false);
    }
    ...
    class BodyPartHealButton extends ButtonWidget {
        ...
        public BodyPartHealButton(BodyPart part, int x, int y, int width, int height, Text message) {
            super(x, y, width, height, message, button -> {
                ClientNetworking.useHealingItem(HealScreen.this.handler.getEntity() ,part.getIdentifier(), HealScreen.this.handler.getItemStack());
